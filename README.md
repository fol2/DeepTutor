...
**OpenAI Codex OAuth (legacy experimental connector).** Tokens live only in `data/system/user-secrets/<owner>/private/openai-codex/` — in the multi-container Compose deployment, outside every tree the exec sandbox can reach — and DeepTutor never reads or modifies `~/.codex`. The connector is owner-bound and never appears in the administrator's cross-account grant catalogue. A consumer ChatGPT subscription is not a DeepTutor runtime credential: OpenAI's current [ChatGPT Pro terms](https://help.openai.com/en/articles/9793128-about-chatgpt-pro-tiers-3) prohibit using Pro to power third-party services. Keep ChatGPT Pro/Codex in OpenAI's official clients and use the OpenAI API or an agreement that expressly covers the integration for DeepTutor.

**Cursor and SuperGrok subscriptions.** The **Cursor Ultra** binding uses the official [`cursor-sdk`](https://cursor.com/docs/sdk/python) with a user API key from Cursor Dashboard → API Keys; SDK requests use the same plan request pools as IDE and Cloud Agent runs. The provider is text-only (`tools=[]`) and selects Cursor Grok 4.6 at high effort from the API key's live model catalogue. The **SuperGrok Heavy** binding uses the locally authenticated Grok CLI for a single `grok-4.6` turn at `high` reasoning. It runs in an isolated workspace with no permitted tools, subagents, memory, plugins, MCP servers, or web search. Both bindings are deployment-owner-only adult operator integrations: they are excluded from cross-account grants and every later administrator's model list and runtime. Use a provider API or another deployment-approved model for an end-user application.
